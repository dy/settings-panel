...
export default function uil({
  ...
  /* native darkens rows by a flat -15/channel step off content (#37383d → #28292e), not a
     ...
     flat-channel trick (+32/channel), so both track their axis instead of sitting fixed. */
  --row: rgb(from var(--bg) calc(r - 15) calc(g - 15) calc(b - 15));
  --face-hover: rgb(from var(--face) calc(r + 32) calc(g + 32) calc(b + 32));

  /* Secondary chrome tones — fixed signature greys of the native skin (not derived from
     shade/text/face), still named tokens so each is overridable on its own. */
  --label: #cccccc;
  --hint: #999;
  --placeholder: #888;
  --on-accent: #fff;
  ...
  > summary, > .s-panel-title {
    ...
    color: var(--text); font-weight: 700; justify-content: flex-start; height: 24px; padding: 0 8px; position: relative;
    &::after { display: none; }
  }
  ...
  .s-slider {
    ...
    .s-tooltip { position: absolute; bottom: 100%; left: 50%; transform: translateX(-50%); margin-bottom: 2px; background: var(--tooltip-bg); color: var(--text); padding: 1px 5px; border-radius: var(--r); white-space: nowrap; }
    &.s-multiple .s-interval-track { height: 22px; margin: 0; border-radius: var(--r); background: var(--well-2); position: relative;
  ...
  .s-boolean {
    ...
    &.s-switch, &.s-toggle {
      .s-track { width: 36px; height: 17px; border-radius: 10px; background: var(--well-3); border: 2px solid var(--well-3); position: relative; cursor: pointer;
        &::after { content: ''; position: absolute; top: 50%; left: 0; width: 16px; height: 13px; transform: translateY(-50%); border-radius: 10px; background: var(--face); transition: left .15s, background .15s; } }
      &:has(input:checked) .s-track { &::after { left: calc(100% - 17px); background: var(--toggle-on); } }
      &:has(input:focus-visible) .s-track { outline: 1px solid var(--accent); outline-offset: 1px; }
    }
    &.s-checkbox { input[type="checkbox"] { -webkit-appearance: none; appearance: none; position: static; opacity: 1; width: 16px; height: 16px; margin: 0; border-radius: 3px; background: var(--well-3); border: 1px solid var(--border); cursor: pointer; &:checked { background: var(--accent); } } .s-track { display: none; } }
  }
  ...
  .s-button {
    .s-input { gap: 4px; }
    button { width: 100%; background: var(--face); color: var(--text); border: 1px solid var(--border); border-radius: var(--r); height: 22px; &:hover { background: var(--face-hover); } &:active, &.s-selected { background: var(--accent); color: var(--on-accent); border-color: var(--accent); } }
  ...
  .s-folder {
    > summary { color: var(--label); font-weight: 400; padding: 4px 4px; background: var(--bg);
      &::after { content: ''; width: 6px; height: 6px; margin-left: auto; background: repeating-linear-gradient(to bottom, var(--text) 0 2px, transparent 2px 4px); transition: transform .15s; } }
  ...
  &::after {
    ...
    height: 24px; background: transparent; color: var(--text); font: inherit;
  }
