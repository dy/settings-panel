...
export default function figma({
  ...
  .s-slider {
    ...
    input[type="range"] {
      ...
      &::-webkit-slider-thumb { -webkit-appearance: none; width: 12px; height: 12px; border-radius: 50%; background: #fff; border: 1px solid rgba(0,0,0,.2); box-shadow: 0 1px 2px rgba(0,0,0,.2); cursor: pointer; transition: box-shadow .12s var(--ease); }
      &::-moz-range-thumb { width: 12px; height: 12px; border-radius: 50%; background: #fff; border: 1px solid rgba(0,0,0,.2); cursor: pointer; transition: box-shadow .12s var(--ease); }
      &:focus-visible::-webkit-slider-thumb { box-shadow: 0 0 0 2px var(--accent), 0 1px 2px rgba(0,0,0,.2); }
  ...
  .s-select {
    ...
    &.s-segmented { .s-input { gap: 0; background: var(--hover); border-radius: var(--r); padding: 2px; }
      button { flex: 1; background: transparent; border: none; color: ${dim}; border-radius: 3px; height: 20px; font: inherit; outline: none;
        ...
        &.s-selected { background: ${dark ? '#2c2c2c' : '#fff'}; box-shadow: 0 1px 2px rgba(0,0,0,.12); color: var(--ink); }
        &.s-selected:focus-visible { box-shadow: 0 1px 2px rgba(0,0,0,.12), inset 0 0 0 1px var(--accent); } } }
  ...
  .s-boolean {
    ...
    &.s-switch {
      .s-track { width: 24px; height: 14px; border-radius: 999px; background: ${dark ? '#555' : '#d9d9d9'}; position: relative; cursor: pointer; transition: background .15s var(--ease);
        &::after { content: ''; position: absolute; top: 2px; left: 2px; width: 10px; height: 10px; border-radius: 50%; background: #fff; box-shadow: 0 1px 2px rgba(0,0,0,.3); transition: left .15s var(--ease); } }
  ...
  .s-color {
    &.s-picker .s-color-input { gap: calc(var(--u) * 1.5); box-sizing: border-box; height: 24px; border: 1px solid transparent; border-radius: 2px; padding: 0 2px 0 5px;
      ...
      &:hover { border-color: var(--line); }
      &:focus-within { border-color: var(--accent); box-shadow: inset 0 0 0 1px var(--accent); }
      input[type="color"] { appearance: none; -webkit-appearance: none; position: static; flex: none; width: 16px; height: 16px; padding: 0; border: 1px solid ${swatchBorder}; border-radius: 3px; box-shadow: ${swatchRing}; cursor: pointer; &::-webkit-color-swatch-wrapper { padding: 0; } &::-webkit-color-swatch { border: none; border-radius: 3px; } }
    ...
    &.s-rgba .s-color-input {
      ...
      gap: calc(var(--u) * 1.5);
      align-items: center;
      input[type="color"] { appearance: none; -webkit-appearance: none; width: 16px; height: 16px; border: 1px solid ${swatchBorder}; border-radius: 3px; box-shadow: ${swatchRing}; &::-webkit-color-swatch-wrapper { padding: 0; } &::-webkit-color-swatch { border: none; border-radius: 3px; } }
      .s-alpha { display: none; }
      input[type="text"] { width: 100%; min-width: 0; padding: 0 2px; text-transform: uppercase; }
    }
    &.s-swatches button { width: 18px; height: 18px; border-radius: 3px; border: 1px solid ${swatchBorder}; box-shadow: ${swatchRing}; outline: none; &.s-selected { outline: 2px solid var(--accent); outline-offset: 1px; } &:focus-visible { outline: 2px solid var(--accent); outline-offset: 1px; } }
  }
  ...
  .s-button {
    ...
    button { flex: 1; height: 32px; background: var(--accent); color: #fff; border: none; border-radius: var(--r); font-weight: 500; padding: 0 12px; outline: none;
